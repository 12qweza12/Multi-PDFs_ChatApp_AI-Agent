...
def user_input(user_question):
    ...
    vector_store = FAISS.load_local("faiss_index", embeddings, allow_dangerous_deserialization=True) #เพิ่ม allow_dangerous_deserialization
    docs = vector_store.similarity_search(user_question)

    chain = get_conversational_chain(vector_store)

    # สร้าง chat_history จาก st.session_state.messages
    # chat_history จะเก็บประวัติการสนทนาในรูปแบบของ tuple (user_message, assistant_message)
    chat_history = [
        (msg["content"], st.session_state.messages[i+1]["content"])
        for i, msg in enumerate(st.session_state.messages[:-1])
        if msg["role"] == "user" and st.session_state.messages[i+1]["role"] == "assistant"
    ]
    ...
    return response["answer"]  # Return the output text for further processing or display
